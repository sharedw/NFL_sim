...
class Play(ABC):
	...
	def sample_oob(self, game_context, play_result):
...
class RunPlay(Play):
	...
	def execute_play(self, team: Team, game_context: dict):
		...
		play_result = PlayResult(**{"yards": int(yds), "rusher_id": rusher.id, "rusher": rusher.name})
...
class PassPlay(Play):
	...
	def update_player_stats(
		...
		receiver = team.get_player_by_id(play_result.receiver_id) #TODO THIS LOOKS WEIRD NOW
		passer.attempts += 1
		receiver.targets += 1
...
class FieldGoal(Play):
	...
	def execute_play(self, team: Team, game_context: dict):
		play_result = PlayResult(**{"yards": 0, "kicker": "dork"})
		self.update_player_stats(team, play_result)
		return play_result
