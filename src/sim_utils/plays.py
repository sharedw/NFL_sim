...
from sim_utils.GameModels import (
	ChooseRusherModel,
	RushYardsModel,
	AirYardsModel,
	YacModel,
...
CHOOSE_RUSHER_MODEL: ChooseRusherModel = ChooseRusherModel(CONFIG)
FG_MODEL = FieldGoalModel(CONFIG)
...
class FieldGoal(Play):
	...
	def sample_result(self, team, game_context) -> int:
		...
		#features = [raw_features[key] for key in FG_MODEL.feature_cols]
		success = FG_MODEL.predict(raw_features)
		return success
	...
	def execute_play(self, team: Team, game_context: dict):
		...
		play_result = PlayResult(
			yards= 0, kicker_id = self.player.id, kicker = self.player.name,
			kick_distance=game_context['yardline_100'] + 17, field_goal_result=success)
		self.update_player_stats(team, play_result)
		return play_result
